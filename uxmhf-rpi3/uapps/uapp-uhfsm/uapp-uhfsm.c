...
bool uapp_uhfsm_va2pa(uint32_t va, u32 *pa){
	u32 par;
...
void uapp_uhfsm_checkacl(uint32_t va){
    u32 paddr;
...
bool uapp_uhfsm_handlehcall(u32  uhcall_function, void *uhcall_buffer, u32 uhcall_buffer_len)
{
  ...
  if(uhcall_function != UAPP_UHFSM_FUNCTION_SIGN)
    return false;

  uhcp=(uhfsm_param_t *)uhcall_buffer;
